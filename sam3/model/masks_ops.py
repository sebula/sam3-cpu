...
def dilation(mask, kernel_size):
    """
    Dilation via separable convolutions on the tensor's device (CPU in this fork).
...
def rle_encode(orig_mask, return_areas=False):
    ...
    # First pass computes RLE run lengths in a flat format
    ...
    # Remaining steps are Python / pycocotools
...
def robust_rle_encode(masks):
    """Encodes masks as RLE; tries the fast path first, then numpy fallback."""
